...
folder="${current_date}_GROK1_CK-MOE-I4F8-AITER-DECODE-ATTN_dummy_offline"
mkdir -p "$folder"
OUTPUT_CSV="${folder}/${current_date}_GROK1_CK-MOE-I4F8-AITER-DECODE-ATTN_dummy_offline.csv"
...
for tp in "${TP_VALUES[@]}"; do
  for bs in "${BATCH_SIZES[@]}"; do
    ...
    if [ -f result.jsonl ]; then
      dest_json="${folder}/${current_date}_GROK1_CK-MOE-I4F8-AITER-DECODE-ATTN_dummy_offline.jsonl"
...
echo "All done! Results saved to ${OUTPUT_CSV} and JSON result stored as ${folder}/${current_date}_GROK1_CK-MOE-I4F8-AITER-DECODE-ATTN_dummy_offline.jsonl (if produced)."
