...
class BenchmarkAnalyzer:
    ...
    def __init__(self, base_dir: Optional[str] = None):
        ...
        self.base_dir = base_dir or os.getenv("BENCHMARK_BASE_DIR", "/mnt/raid/michael/sgl_benchmark_ci")
        self.offline_dir = os.path.join(self.base_dir, "offline")
        self.online_dir = os.path.join(self.base_dir, "online")

    def parse_gsm8k_accuracy(self, model: str, mode: str, date_str: str) -> Optional[float]:
        """
    ...
    def compare_performance_metrics(self, model: str, mode: str, current_date: str, days_back: int = 7) -> Dict:
        """
        ...
        baseline_metrics, baseline_date = self._find_baseline_metrics(model, current_date, days_back)
    ...
    def _find_baseline_metrics(self, model: str, current_date: str, days_back: int) -> Tuple[Dict, str]:
...
class TeamsNotifier:
    ...
    def __init__(self, webhook_url: str, plot_server_base_url: str, skip_analysis: bool = False, analysis_days: int = 7, benchmark_dir: Optional[str] = None):
        """
    ...
    def create_summary_alert(self, model: str, mode: str) -> Dict:
        ...
        if gsm8k_accuracy is not None:
            ...
            if gsm8k_accuracy < threshold:
                ...
                alert['details'].append(f"GSM8K accuracy: {gsm8k_accuracy:.1%} (below {threshold:.1%} threshold)")
            else:
                alert['details'].append(f"GSM8K accuracy: {gsm8k_accuracy:.1%} ✅")

        # Check performance regressions (online mode only)
        if mode == 'online':
            perf_results = self.analyzer.compare_performance_metrics(model, mode, current_date, self.analysis_days)
            ...
            if perf_results['has_regression']:
                ...
                for regression in perf_results['regressions']:
                    ...
                    alert['details'].append(f"{metric_name}: +{change_pct:.1f}% vs {regression['baseline_date']} ⚠️")
            elif perf_results['current_metrics']:
                alert['details'].append("Performance metrics: No regression detected ✅")
    ...
    def discover_plot_files(self, model: str, mode: str, plot_dir: str) -> List[Dict[str, str]]:
        """
    ...
    def create_adaptive_card(self, plots: List[Dict[str, str]], model: str, mode: str) -> Dict:
        """
    ...
    def send_notification(self, plots: List[Dict[str, str]], model: str, mode: str) -> bool:
        """
        ...
        try:
            ...
            if response.status_code in [200, 202]:
                print(f"✅ Successfully sent Teams notification for {model} {mode} plots")
                if response.status_code == 202:
                    print("   (Power Automate flow accepted - message processing asynchronously)")
                return True
            else:
...
def main():
    ...
    notifier = TeamsNotifier(webhook_url, plot_server_base_url, args.skip_analysis, args.analysis_days, args.benchmark_dir)
    ...
    if success:
        if args.skip_analysis:
            ...
            print("🎉 Teams notification sent successfully! (with intelligent analysis)")
        return 0
    else:
