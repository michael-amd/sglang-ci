...
#   python3 sanity_check.py --docker-image=rocm/sgl-dev:v0.5.3rc0-rocm700-mi30x-20250925 --hardware=mi30x
#
...
GPU_USAGE_THRESHOLD = int(os.environ.get("GPU_USAGE_THRESHOLD", "5"))    # GPU usage threshold in %
VRAM_USAGE_THRESHOLD = int(os.environ.get("VRAM_USAGE_THRESHOLD", "5"))  # VRAM usage threshold in %
...
def check_gpu_idle():
    ...
    try:
        ...
        if result.returncode != 0:
            print("[sanity] WARN: rocm-smi not found or failed. Skipping GPU idle check.")
            return True
        ...
        for line in lines[2:]:  # Skip header lines
            ...
            if len(fields) >= 2:
                try:
                    ...
                    if gpu_usage > GPU_USAGE_THRESHOLD or vram_usage > VRAM_USAGE_THRESHOLD:
                        return False
...
def cleanup_sglang_processes():
    ...
    try:
        result = subprocess.run(["lsof", "-ti:30000"], capture_output=True, text=True, timeout=10)
    ...
    try:
        result = subprocess.run(["pgrep", "-f", "sglang.launch_server"], capture_output=True, text=True, timeout=10)
        if result.returncode == 0 and result.stdout.strip():
            pids = result.stdout.strip().split('\n')
            ...
            for pid in pids:
                if pid:
                    try:
                        ...
                        subprocess.run(["kill", "-15", pid], capture_output=True, timeout=5)  # SIGTERM
                        time.sleep(2)
                        # Check if still running, then force kill
                        check_result = subprocess.run(["kill", "-0", pid], capture_output=True, timeout=5)
                        if check_result.returncode == 0:
                            subprocess.run(["kill", "-9", pid], capture_output=True, timeout=5)  # SIGKILL
    ...
    try:
        ...
        result = subprocess.run(["pgrep", "-f", "python.*sglang"], capture_output=True, text=True, timeout=10)
        if result.returncode == 0 and result.stdout.strip():
            pids = result.stdout.strip().split('\n')
            ...
            if filtered_pids:
                print(f"[sanity] Found Python SGLang processes: {' '.join(filtered_pids)}")
                ...
                print("[sanity] Found Python SGLang processes, but they are this script - skipping self-termination")
    except (FileNotFoundError, subprocess.TimeoutExpired):
        ...
        if subprocess.run(["which", "docker"], capture_output=True).returncode == 0:
            try:
                ...
                if result.returncode == 0:
                    ...
                    result = subprocess.run(["docker", "ps", "-q"], capture_output=True, text=True, timeout=15)
                    if result.returncode == 0:
                        running_ids = result.stdout.strip().split('\n')
                        running_ids = [cid for cid in running_ids if cid]  # Filter empty strings
                        
                        if running_ids:
                            print(f"[sanity] Stopping running containers: {' '.join(running_ids)}")
                            subprocess.run(["docker", "stop"] + running_ids, capture_output=True, timeout=60)
                        else:
                            print("[sanity] No running containers to stop.")
                    else:
                        print("[sanity] WARN: Failed to get running container list.")
                else:
                    print("[sanity] WARN: Docker not accessible; skipping container stop.")
...
def manage_docker_container(docker_image, work_dir, model_path, script_path, models_dir=DEFAULT_MODELS_DIR):
    ...
    if not subprocess.run(["which", "docker"], capture_output=True).returncode == 0:
        ...
        return None  # No docker available, assume we're already in the right environment
    ...
    if container_name in container_exists:
        ...
        script_accessible = subprocess.run(
            ["docker", "exec", container_name, "test", "-f", script_path],
            capture_output=True
        ).returncode == 0
        
        model_accessible = subprocess.run(
...
def sanity_check(model_name, config, platform, trials=3, log_dir=None, model_path=None, tokenizer_path=None, timing_log=None, docker_image=None):
    ...
    if not launch_cmd:
        ...
        if timing_log:
            timing_log.write(f"{model_name}: SKIPPED (no launch command for {platform})\n")
        return False
    ...
    with open(server_log, "w") as f:
        server_proc = subprocess.Popen(
            launch_cmd, shell=True, stdout=f, stderr=subprocess.STDOUT,
    ...
    if not ready:
        print(f"{model_name}: {FAIL_MARK} (server not ready after {server_ready_time:.2f}s)")
        if timing_log:
    ...
    for i in range(1, trials + 1):
        ...
        with open(client_log, "w") as f:
            proc = subprocess.Popen(bench_cmd, shell=True, stdout=f, stderr=subprocess.STDOUT, text=True)
            
            # Show progress while benchmark is running
            while proc.poll() is None:
                elapsed_so_far = time.time() - trial_start
                if elapsed_so_far > 30 and int(elapsed_so_far) % 15 == 0:  # Show progress every 15s after 30s
                    print(f"    ⏳ Trial {i} still running... ({elapsed_so_far:.0f}s elapsed)")
                time.sleep(1)
        ...
        try:
            ...
            print(f"  ✅ Trial {i}: {status} (Accuracy: {acc:.3f}, Time: {elapsed:.2f}s)")
            if timing_log:
                timing_log.write(f"Trial {i}: {status} (Accuracy: {acc:.3f}, Time: {elapsed:.2f}s)\n")
                timing_log.flush()
        except ValueError as e:
            print(f"  ❌ Trial {i}: {FAIL_MARK} ({e}, Time: {elapsed:.2f}s)")
            if timing_log:
                timing_log.write(f"Trial {i}: {FAIL_MARK} ({e}, Time: {elapsed:.2f}s)\n")
                timing_log.flush()
            accuracies.append(0.0)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SGLang Sanity Check with Docker Support")
    ...
    parser.add_argument("-p", "--platform", choices=["mi30x", "mi35x"], help="Target platform (legacy)")
    ...
    if args.model_path and args.model_type:
        ...
        models_to_test = {args.model_type: {
            ...
            "launch_cmd_template": DEFAULT_MODELS.get(args.model_type, {}).get("launch_cmd_template", {}),
            "bench_cmd": "python3 /sgl-workspace/sglang/benchmark/gsm8k/bench_sglang.py --num-questions 2000 --parallel 2000",
            "criteria": DEFAULT_MODELS.get(args.model_type, {}).get("criteria", {"accuracy": 0.5})
        }}
    ...
    if args.log_dir:
        ...
        base_log_dir = os.path.abspath(DEFAULT_LOG_DIR)
    
    # Create subfolder based on Docker image tag if provided
    if args.docker_image:
        image_tag = args.docker_image.split(':')[-1] if ':' in args.docker_image else args.docker_image.split('/')[-1]
    ...
    timing_log_path = os.path.join(log_dir_path, f"timing_summary_{time.strftime('%Y%m%d_%H%M%S')}.log")
    ...
    try:
        ...
        print(f"💡 Try running with sudo or use a different log directory with --log-dir")
        sys.exit(1)
    ...
    with open(timing_log_path, 'w') as timing_log:
        ...
        for model_name, config in models_to_test.items():
            ...
            print(f"Progress: [{progress_bar}] {current_model}/{total_models} ({(current_model-1)/total_models*100:.1f}%)")
            
            results[model_name] = sanity_check(
                model_name, config, platform, 
                trials=args.trials,
            ...
            print(f"Progress: [{progress_bar}] {current_model}/{total_models} ({current_model/total_models*100:.1f}%) {status_emoji} {model_name} completed in {model_duration:.1f}s")
        ...
        timing_log.write("=" * 50 + "\n")
        timing_log.write(f"End time: {time.strftime('%Y-%m-%d %H:%M:%S %Z')}\n")
        timing_log.write(f"Total execution time: {total_script_time:.2f}s ({total_script_time/60:.1f} minutes)\n")
        timing_log.write(f"Average time per model: {total_script_time/total_models:.2f}s\n")
    ...
    print(f"\n🎯 All tests completed!")
    print(f"📊 Total execution time: {total_script_time:.2f}s ({total_script_time/60:.1f} minutes)")
